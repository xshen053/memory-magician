 ...
 */
export const generateAllReviewDates = (todayDateString, intervals = [0, 1, 5, 10, 20, 30, 45, 60, 90, 120, 150]) => {
  return intervals.map(interval => {
    let date = new Date(todayDateString); // Create a new Date object for each interval
    date.setUTCDate(date.getUTCDate() + interval); // Add the interval to the new date
